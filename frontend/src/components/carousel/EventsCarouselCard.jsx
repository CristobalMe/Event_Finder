    ...
    return(
      <Link to={`/event/${data.id}`}>
        <div className="max-w-sm rounded overflow-hidden shadow-lg bg-white h-[20rem] w-[15rem] hover:bg-neutral-300">
          <img className="h-[10rem] w-[15rem]" src={data.image} id="image" onError={e => {e.target.src = "https://www.shutterstock.com/image-vector/default-ui-image-placeholder-wireframes-600nw-1037719192.jpg"}} />
          <div className="px-3 py-4 flex flex-col justify-center items-center">
            ...
            <div className="flex items-center mt-[2.5rem]">
                  ...
                  <p className="ms-2 text-sm font-bold text-gray-900 dark:text-white">{data.rating}</p>
            </div>
          </div>
        </div>
      </Link>
